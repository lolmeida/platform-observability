...
The GitHub repository rename to `platform-quarkus` is a separate operational change. Verify GitHub Packages Maven resolution for the old endpoint after the rename before changing consumer repository URLs. If the old endpoint is not reliable, publish `0.2.0` to the new endpoint and migrate consumers in separate pull requests. No consumer migration belongs in this change.
